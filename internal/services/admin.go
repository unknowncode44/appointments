...
func (s *adminService) CreateTenant(ctx context.Context, req dto.TenantRequest) (dto.TenantResponse, error) {
	...
	if err != nil {
		// tenants only has the slug unique index, so a unique violation here is
		// unambiguously a duplicate slug.
		if db.ErrorCode(err) == db.UniqueViolation {
			return dto.TenantResponse{}, response.ErrSlugTaken
		}
		return dto.TenantResponse{}, err
	}
	return mapTenant(item), nil
}
